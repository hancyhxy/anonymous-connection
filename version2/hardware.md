...
| # | 标注 | 可用功能 | 备注 |
|---|---|---|---|
| 1 | IO12 | ⚠️ **USB_D-，禁用作 GPIO** | ESP32-C6 上 IO12 = USB D- 数据线。`pinMode(12, ...)` 一执行 USB CDC 立即失效、Mac 端 `/dev/cu.usbmodem*` 消失、烧录失败。**不要接按钮、不要接任何外设。** |
| 2 | IO13 | ⚠️ **USB_D+，禁用作 GPIO** | 同上，IO13 = USB D+。 |
| 3 | IO16 | I2C / I2S / PWM / UART | 数字 IO，**按钮首选** |
| 4 | IO17 | I2C / I2S / PWM / UART | 数字 IO，备用 |
| 5 | IO20 | I2C / I2S / PWM / UART | 数字 IO，备用 |
| 6 | IO23 | I2C / I2S / PWM / UART | 数字 IO，备用 |
...
| 用途 | GPIO | 阶段 | 备注 |
|---|---|---|---|
| 配对按钮 / 推进按钮 | **IO16**（推荐） | Phase 1+ | 内部上拉 + 按下接 GND（3-pin 模块 S→IO16、−→GND，中间空脚不接）。**不要用 IO12/13**，那是 USB 数据线 |
